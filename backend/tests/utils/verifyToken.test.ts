...
describe("verifyToken", () => {
  ...
  it("Shoul return status 401 if the token is not sent", async () => {
  ...
  it("Shoul return status 401 if the token's format is incorrect", async () => {
  ...
  it("Should return status 403 if the toke is not valid", async () => {
    const response = await request(app)
      .get(ENDPOINT)
