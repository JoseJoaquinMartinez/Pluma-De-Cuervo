...
describe("POST postCommentResponseByAdmin", () => {
  ...
  it("Should return status 404 if hte comment does not exist", async () => {
  ...
  it("Should create a reply comment if the admin exist and the comment exists", async () => {
    const response = await request(app)
      .post(ENDPOINT)
      .set("Authorization", `Bearer ${validToken}`)
      .send({ commentBody: "Test comment " });
    expect(response.status).toBe(200);
    expect(response.body).toMatchObject(MOCKREPLYCOMMENT);
  });
